...
class machine_model extends CI_Model {
    ...
    public function saveTemplate()
    {
        ...
        $this->input->post("max_temperature") != ""
        ){
            // Save Detail Table
            $date = date_create();
        ...
        if($imageFile != null){
            ...
            if($this->input->post("choiceTemplates") == "new"){
                ...
                if($this->input->post("templatePicture_o") != "noimage2.jpg"){
                    ...
                    $pathSource = $_SERVER['DOCUMENT_ROOT']."/intsys/msd_oven/".$rsImagePath.$imageOld;
                    $pathDestination = $_SERVER['DOCUMENT_ROOT']."/intsys/msd_oven/".$rsImagePath.$rsImageFile;
                    // Copy Image
            ...
            $arInsertTemplate = array(
                ...
                "master_max_temperature" => $this->input->post("max_temperature"),
    ...
    public function loaddataToEditModal()
    {
        ...
        // Select Detail Data
            ...
            $output = array(
                ...
                "master_max_temperature" => $sqlSelectMainTable->row()->master_max_temperature,
    ...
    private function saveEditMasterTable($fileInput , $master_temcode)
    {
        ...
        if($_FILES[$fileInput]['tmp_name'] != ""){
            ...
            if(getFileForCheck($master_temcode) == "yes"){
                $path = $_SERVER['DOCUMENT_ROOT']."/intsys/msd_oven/".$this->input->post("imagePath_edit").$this->input->post("imageFile_edit");
                unlink($path);
            }
            ...
            $arInsertTemplate = array(
                ...
                "master_max_temperature" => $this->input->post("temperature_edit"),
    ...
    public function deleteTemplate()
    {
        ...
        if($received_data->action == "deleteTemplate"){
            ...
            if($received_data->imageFile != "noimage2.jpg"){
                // Unlink File
                $path = $_SERVER['DOCUMENT_ROOT']."/intsys/msd_oven/".$received_data->imagePath.$received_data->imageFile;
                unlink($path);
            }
            ...
            if($getOtherImage->num_rows() != 0){
                foreach($getOtherImage->result() as $rs){
                    $path = $_SERVER['DOCUMENT_ROOT']."/intsys/msd_oven/".$rs->tm_imagepath.$rs->tm_imagename;
                    @unlink($path);
                }
    ...
    public function delOtherImage()
    {
        ...
        if($received_data->action == "delOtherImage"){
            ...
            $path = $_SERVER['DOCUMENT_ROOT']."/intsys/msd_oven/".$filepath.$filename;
            unlink($path);
